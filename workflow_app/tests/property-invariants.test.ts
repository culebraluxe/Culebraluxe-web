...
import { parallelShapeRefusal } from '../../db/agent-work'
import { sprintCloseRefusal } from '../../db/sprint'
import {
  normalizeEmail,
  normalizePhone,
...
test('suite-runs-without-database', () => {
  const script = [
    "Promise.all([import('./db/agent-work.ts'),import('./db/sprint.ts'),",
    "import('./lib/relationship-intel/normalize.ts'),import('./db/person-identities.ts'),",
    "import('./lib/storyboard-data.ts')]).then(([a,s,n,p,d])=>{",
