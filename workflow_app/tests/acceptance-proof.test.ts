...
import {
  ...
  type CommandResult,
} from '../forge/agents/qa/types'
import { storyReadyToRunReasons } from '../forge/forge-ready-gate'
import { buildStoryAcceptanceMap } from '../forge/lead-routing-context'
...
test('the pre-work mapping carries a declared assertion per clause and leaves the rest unmapped', () => {
  const map = buildStoryAcceptanceMap({
    ...
    architectBrief:
      'FORGE_ARCHITECT_CONTRACT: {"filesInScope":["workflow_app/tests"],"acceptance":["one clause","another clause"],"acceptanceAssertions":{"one clause":["asserts-one"]},"leadHint":"SMITH"}',
  })
