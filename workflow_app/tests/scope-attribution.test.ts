...
import { candidateOwnChangedFiles } from '../forge/story-scope-base'
import { scopeViolations, type SmithExecutionContract } from '../forge/smith-contract'
import { serialScopeMissReasons } from '../forge/forge-serial-doors'
...
const contract = (over: Partial<SmithExecutionContract> = {}): SmithExecutionContract => ({
  ...
  requiredInputs: [],
  allowedScope: ['workflow_app/forge/a.ts'],
  prohibitedScope: [],
  expectedOutputs: [],
  requiredEvidence: [],
  dependsOn: [],
...
async function makeRange() {
  ...
  const baseSha = await commit(dir, 'workflow_app/forge/base.ts', 'export const x = 1\n', 'base')
  const foreignSha = await commit(dir, 'docs/foreign.md', '# foreign\n', 'foreign: operator commit')
  const cleanSha = await commit(dir, 'workflow_app/forge/a.ts', 'export const a = 1\n', 'lane: clean')
...
test('candidateOwnChanges excludes a foreign commit that landed in the range', async () => {
  ...
  if (!own.ok) return
  assert.deepEqual(own.changedFiles, ['workflow_app/forge/a.ts'])
...
test('candidateOwnChanges includes every commit the lane authored in the range', async () => {
  ...
  assert.equal(own.ok, true)
  if (!own.ok) return
  assert.deepEqual(own.changedFiles, ['app/api/evil.ts', 'workflow_app/forge/a.ts'])
  assert.ok(!own.changedFiles.includes('docs/foreign.md'))
})
