...
test('failAgentWork records Error without touching the story', async () => {
  const f = new FakeQueueDb()
  const id = await seedStory(f)
  await setStoryboardStatus(id, 'Ready', f.tx)
  const claim = await claimNextAgentWork('worker-1', f.runner)
...
test('cancelAgentWork marks an active item Cancelled', async () => {
  const f = new FakeQueueDb()
  const id = await seedStory(f)
  await setStoryboardStatus(id, 'Ready', f.tx)
  const claim = await claimNextAgentWork('worker-1', f.runner)
  ...
  const cancelled = await cancelAgentWork(claim.workItem.id, f.tx)
