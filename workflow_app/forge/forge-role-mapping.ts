    ...
    case 'lead_pre':
      return {
        ...
        evidenceInstruction:
          ...
          'A chunk with no surface or no proof is REFUSED by the database — that is the point. Base every chunk on the story\'s ACTUAL goal + final acceptance + architect brief and the Scout surfaces in your context — never invent scope. Maximum 3 chunks; a fourth is a recut, not a plan. (A LEAD_PLAN JSON line is still accepted as a fallback, but the rows are the contract.)\n' +
