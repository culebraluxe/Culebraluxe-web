...
export function buildLeadRoutingDirective(context: LeadRoutingDirectiveContext): string {
  ...
  return [
    ...
    'ASSERTION REF FORMAT: write the ref as a bare `<name>`, or file-qualified `path#name` when the assertion must come from a specific file. A bare name is accepted only while it is UNAMBIGUOUS in the executed output (one origin, one verdict). A file-qualified ref is answered ONLY by the file it names: the reader takes the originating file from the runner’s structured output (the JUnit reporter names the file for every test, passing included), so a proof invoked with a reporter that prints no file provenance leaves a qualified ref UNPROVEN — and an assertion that ran in a DIFFERENT file never satisfies it. When you qualify a ref, the frozen proof must be able to name the file: prefer a bare ref otherwise.',
