...
test('handoff maps scope onto live seams', () => {
  const raw = `prose\nFORGE_ARCHITECT_HANDOFF: {"version":1,"baseRef":"abc","findings":[{"id":"F1","required":true,"summary":"x","preconditions":["p"],"scope":["workflow_app/forge/a.ts"],"postconditions":["q"],"classes":["T"],"risks":["r"],"hint":"SAME_UNIT"}]}`
  const handoff = parseArchitectHandoff(raw)
  assert.ok(handoff)
  const findings = handoffToFindings(handoff!)
  assert.equal(findings[0].seams[0], 'workflow_app/forge/a.ts')
