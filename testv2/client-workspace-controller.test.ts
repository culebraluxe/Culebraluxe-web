...
//
// The MVI "LEGO" claim, proven: the ClientWorkspaceController is a framework-neutral
// pure class. We drive it here from `node:test` with a controllable fake
// `ClientWorkspaceSource` (no React, no DOM, no HTTP, no DB) and assert on the
// published PageModel. This is the same glass-box shape as the service tier —
// swap the fake for `HttpClientWorkspaceSource` and you get the black-box tier.
//
...
import { ClientWorkspaceController } from '../ui/client-workspace/client-workspace-controller'
import type { ClientWorkspaceSource } from '../ui/client-workspace/source'
...
type FakeSource = ClientWorkspaceSource & { saveCalls: number; listCalls: number }

/** Build a controllable fake source; each method can be overridden per test. */
function makeFake(over: Partial<ClientWorkspaceSource> = {}): FakeSource {
...
test('clientWorkspace.load lists the rail, auto-selects the first client, and fans out Person/Property/channel lanes', async () => {
  ...
  const controller = new ClientWorkspaceController(fake)
  await controller.dispatch({ operation: 'clientWorkspace.load', payload: {} })
  await settle()
...
test('a superseded list load cannot overwrite a newer response (latest-wins on the rail)', async () => {
  ...
  void controller.dispatch({ operation: 'clientWorkspace.load', payload: {} }) // call 1 (stale, held)
  const secondDone = controller.dispatch({ operation: 'clientWorkspace.load', payload: {} }) // call 2 (current)
...
test('selectClient switches selection and ignores late lane writes from a stale selection', async () => {
  ...
  const controller = new ClientWorkspaceController(fake)
  await controller.dispatch({ operation: 'clientWorkspace.selectClient', payload: { personId: 'a' } })
  // Alice's Person lane is still in flight.

  await controller.dispatch({ operation: 'clientWorkspace.selectClient', payload: { personId: 'b' } })
  await settle()
...
test('notes: a changed draft saves, the clean save is a no-op against the source, failure surfaces', async () => {
  ...
  await controller.dispatch({ operation: 'clientWorkspace.notesChanged', payload: { notes: 'prefers weekends' } })
  assert.equal(controller.snapshot().notesDraft, 'prefers weekends')

  await controller.dispatch({ operation: 'clientWorkspace.saveNotes', payload: {} })
  await settle()
...
test('a failed save surfaces notesStatus without corrupting the draft', async () => {
  ...
  await controller.dispatch({ operation: 'clientWorkspace.selectClient', payload: { personId: 'a' } })
  await settle()
  await controller.dispatch({ operation: 'clientWorkspace.notesChanged', payload: { notes: 'draft text' } })
...
test('pagination clamps at page 1 and queryChanged resets page to 1', async () => {
  ...
  await controller.dispatch({ operation: 'clientWorkspace.previousPage', payload: {} })
  await settle()
  assert.equal(controller.snapshot().page, 1)

  // A new query resets to page 1 immediately (no source round-trip needed).
  await controller.dispatch({ operation: 'clientWorkspace.queryChanged', payload: { query: 'ali' } })
