...
import { projectClientWorkspaceChannels } from '../ui/client-workspace/channel-projection'
...
test('empty channel set projects six canonical slots, all disconnected', () => {
  const slots = projectClientWorkspaceChannels([])
...
test('one channel per canonical source maps to the expected slot with its counters', () => {
  ...
  const slots = projectClientWorkspaceChannels(rows)
  assert.equal(slots.length, 6)
...
test('slot classification is exclusive (no double counting) for ambiguous sources', () => {
  ...
  const slots = projectClientWorkspaceChannels([chan(P, 'facetime', 'call', { totalCount: 1 })])
  ...
  const cal = projectClientWorkspaceChannels([chan(P, 'calendar', 'email', { totalCount: 1 })])
