...
**Canonical TUNIT run command (ENG-04)**

One command runs the whole durable regression suite from a clean checkout, no
database required:
...
- `pnpm test:engine` — `workflow_engine/tests/*.test.ts` (44 in-memory tests)
- `pnpm test:app` — `workflow_app/tests/*.test.ts` (155 in-memory tests)
- Both tiers run on in-memory fakes; no DB/Neon needed and no `.env.local`
  required (verified from a clean checkout).
- The engine/app architectural boundary is preserved: no engine test imports
  application code.
...
| 2 | Neon interactive transaction surface (nested-fragment flattening + lazy thenable memoization) | `APPLICATION INTEGRATION` | [`workflow_app/tests/neon-interactive.test.ts`](workflow_app/tests/neon-interactive.test.ts) (8 durable tests, no DB); live `begin`/COMMIT/ROLLBACK atomicity remains a `LIVE DEV` marker (CRM-14H/J) |
...
- `GLOBAL INVARIANT` checks are dependency-injected detectors in
  [`workflow_app/anomaly-core.ts`](workflow_app/anomaly-core.ts)
  (`failed-process`, `pending-receipt`, `ready-task-uncorrelated`,
  `correlation-dangling-*`, `open-job-on-closed-token`,
  `multiple-active-instances`), wired into the live read-only sweep in
  [`workflow_app/diagnostics.ts`](workflow_app/diagnostics.ts); the terminal
  sweep is a composition of these.

## Known remaining gap (not a blocker)

The transactional concurrency of the **join release** under two simultaneous
branch completions was the original gap. It is now covered by
[`workflow_app/tests/concurrency.test.ts`](workflow_app/tests/concurrency.test.ts)
(S-028) — two required fork branches completing concurrently release the join
exactly once and produce exactly one downstream token.
