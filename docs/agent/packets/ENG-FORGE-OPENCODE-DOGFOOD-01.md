...
Harden the OpenCode evidence boundary so a model run cannot be treated as a clean Smith success when its captured output contains deterministic failed-test evidence even if the OpenCode CLI itself exits zero. Do not make prose authoritative; use bounded machine-recognizable failure evidence and preserve deterministic Assay as the final exact-SHA authority.
...
- [ ] OpenCode evidence cannot report a clean Smith success when captured machine-readable test output contains a failed test/assertion.
...
- targeted OpenCode harness tests only
