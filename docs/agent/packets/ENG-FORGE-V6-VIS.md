...
## Scope
Read-only visibility over existing seams only: `legacy/db/forge-run.ts` (`getForgeRunExecutionStory`, `getForgeRunMachineEvidence`, `getForgeLeadRunRecord`), `legacy/db/storyboard.ts` (`getStoryboardStory`, `listStoryRuns`), `lib/worker-workspace` (`resolveApprovedBaseRef`, `listWorkerWorkspaces`). Swarm stays V7: `SPLIT:n` remains parsed but Hold-gated, no auto-prune on publish (orphan detector + manual `remove` only).
...
## Assay commands
- `pnpm exec tsx --test agent-runtime/slack-notifier.test.ts legacy/workflow_app/tests/workspace-cli.test.ts`
