...
-- (by provenance source_system) that carries a human-readable title. NULL when
-- a source has no such context. This is relationship memory, not a message
-- archive — never thousands of excerpts.
ctx as (
    -- Latest useful bounded title OR summary for this Person+source.
    select distinct on (i.person_id, coalesce(i.source_system, ''))
        i.person_id,
        coalesce(i.source_system, '') as source,
        (case
            when i.title is not null and btrim(i.title) <> '' then i.title
            else i.summary
        end) as last_context,
        (case
            when i.title is not null and btrim(i.title) <> '' then 'title'
            else 'summary'
        end) as last_context_type,
        i.occurred_at
    from interaction i
    where (i.title is not null and btrim(i.title) <> '')
       or (i.summary is not null and btrim(i.summary) <> '')
    order by i.person_id, coalesce(i.source_system, ''), i.occurred_at desc
)
select
    a.person_id,
    a.source,
