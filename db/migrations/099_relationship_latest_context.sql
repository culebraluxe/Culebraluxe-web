-- 099_relationship_latest_context.sql
-- Restore the locked Cloze-style Client relationship-memory contract:
-- one Person x source row with the newest human-readable bounded context and
-- that context event's own timestamp/direction.
...
select
    ...
    case
        ...
        when a.source in ('gmail_contacts', 'gmail') then 'email'
        else a.source
    end as channel,
    a.first_observed_at,
    a.last_contact_at,
