...
export type ClientWorkspaceLoadListRequest = {
  query: string
  page: number
  pageSize: number
}

export type ClientWorkspaceLoadOptions = {
  signal?: AbortSignal
}

export interface ClientWorkspaceSource {
  loadList(
    request: ClientWorkspaceLoadListRequest,
    options?: ClientWorkspaceLoadOptions,
  ): Promise<ClientWorkspaceListPage>
  loadClient(personId: string, options?: ClientWorkspaceLoadOptions): Promise<Client | null>
  loadChannels(
    personId: string,
    options?: ClientWorkspaceLoadOptions,
...
type DirectoryResponse = ClientWorkspaceListPage
...
export class HttpClientWorkspaceSource implements ClientWorkspaceSource {
  async loadList(
    request: ClientWorkspaceLoadListRequest,
    options: ClientWorkspaceLoadOptions = {},
  ): Promise<ClientWorkspaceListPage> {
...
export class InMemoryClientWorkspaceSource implements ClientWorkspaceSource {
  ...
  async loadList(request: ClientWorkspaceLoadListRequest): Promise<ClientWorkspaceListPage> {
    ...
    const rows: ClientWorkspaceListItem[] = all
  ...
  async loadPropertyContext(personId: string): Promise<PersonPropertyContextDto> {
    ...
    return context ?? { personId, properties: [] }
  }
