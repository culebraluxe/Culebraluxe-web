...
import {
  confirmPlacementAction, publishListingsAction, withdrawPlacementAction,
  type MarketingWriteState,
} from '@/app/portal/marketing/actions'
import {
  CHANNEL_CATALOG, SYNDICATION_CHANNELS, type SyndicationChannel,
...
export function SyndicationWorkbench({ sources, placements }: {
  sources: ListingSource[]; placements: PlacementRow[]
}) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[0.9fr_1.2fr]">
        ...
        <div className="space-y-4">
          <Panel eyebrow="Targets" heading="Channels" subtitle="Same listing, many paths. Blocked adapters stay visible.">
            {source ? (
                    ...
                    return (
                      <li key={id} className="rounded-[var(--portal-panel-radius)] border border-[var(--portal-panel-border)] bg-white/40 p-3">
                        <label className="flex items-start gap-2">
                          <input type="checkbox" name="channel" value={id} disabled={!selectable} checked={selectable ? checked : false} onChange={(event) => {
