  ...
  if (phase === 'pre') {
    return withLeadRunPhaseDirective(
      [
        ...
        'LEGACY PATH: live PRE uses LEAD_ROUTING (see forge-lead-routing-prompt). Only emit LEAD_DECISION here if no LEAD_ROUTING context was supplied.',
        'Shape the Architect findings before choosing: one bounded implementation unit -> SMITH/SOLO. Do not emit SPLIT unless the trusted runtime context explicitly enables it. Never collapse N independent seams into a single SMITH when they cannot share one worker context — HOLD and recut instead.',
        'Choose the cheapest sound execution shape. Default to NOT splitting. SOLO when this is small enough for Lead to implement directly. SMITH for one coherent implementation assignment (1..3 serial chunks). SPLIT is a runtime capability, not a prompt wish.',
        ...
        'End with exactly one machine line: LEAD_DECISION: SOLO | SMITH | HOLD',
        'Then one concise line: LEAD_REASON: <why>.',
        'Do not emit LEAD_DECISION: SPLIT:n on this legacy path. Split is owned by LEAD_ROUTING + runtime splitEnabled.',
      ].join('\n'),
      phase,
    )
  }
