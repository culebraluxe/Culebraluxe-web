  ...
  if (phase === 'pre') {
    return withLeadRunPhaseDirective(
      [
        ...
        'Shape the Architect findings before choosing: one bounded implementation unit -> SMITH/SOLO; genuinely independent required seams -> the runtime decides whether a split is dispatchable. Never collapse N independent seams into a single SMITH — the shaping gate refuses that. Architect discovery beyond the current story is follow-up material, never current Smith scope.',
        'Choose the cheapest sound execution shape. Default to NOT splitting. SOLO when this is small enough for Lead to implement directly. SMITH for one coherent implementation assignment. The runtime owns whether SPLIT is available and how many workers exist — do not infer it from this text, and do not emit a split you cannot justify.',
