...
const cleanInput = {
  resultStatus: 'Complete', completion: 100, notes: 'verifier finished',
  commitHash: null, testsSummary: 'candidate-assay-handoff.test.ts 18/18 pass',
}
...
test('missing-file and non-zero command evidence is failure evidence', () => {
  ...
  for (const dirty of ['1 failed, 11 passed', 'agent-runtime/slack-notifier.test.ts does not exist in this checkout', 'No test files found for slack-notifier.test.ts', 'command not found: pnpm', 'exit code 1 from the Assay command', 'file is missing from the workspace', 'candidate could not resolve to a commit', 'verification policy gate rejected']) {
...
test('candidateVerifiedEvidenceLine records the exact verified SHA', () => {
  assert.equal(candidateVerifiedEvidenceLine(CANDIDATE), `Assay verified candidate ${CANDIDATE} (workspace base == candidate).`)
})

test('Assay lanes resolve their workspace base to the exact candidate commit', () => {
  for (const role of ['reviewer', 'verifier']) assert.deepEqual(resolveAssayWorkspaceBase({ role, candidateSha: CANDIDATE, fallbackBaseRef: 'main' }), { baseRef: CANDIDATE })
})

test('non-Assay lanes keep the existing approved integration base', () => {
  for (const role of ['builder', 'scout', 'architect', null]) assert.deepEqual(resolveAssayWorkspaceBase({ role, candidateSha: CANDIDATE, fallbackBaseRef: 'main' }), { baseRef: 'main' })
})

test('an Assay lane with no resolvable candidate fails closed (never main)', () => {
  for (const candidateSha of [null, '', '   ', 'not-a-commit']) {
    const resolved = resolveAssayWorkspaceBase({ role: 'verifier', candidateSha, fallbackBaseRef: 'main' })
    assert.ok('error' in resolved, String(candidateSha))
    if ('error' in resolved) assert.match(resolved.error, /refusing to provision from main/)
  }
})

test('V4-11: verifier Assay provisioned from main cannot normalize to Complete', () => {
  const normalized = normalizeAgentFinishForRole('verifier', { ...cleanInput, notes: notesWithBase(MAIN_BASE, 'main') }, context(CANDIDATE))
...
test('reviewer role gets the same wrong-base Hold semantics', () => {
  const normalized = normalizeAgentFinishForRole('reviewer', { ...cleanInput, notes: notesWithBase(MAIN_BASE, 'main') }, context(CANDIDATE))
  assert.equal(normalized.resultStatus, 'Hold')
  assert.match(normalized.notes, /does not equal Smith candidate/)
})

test('clean Assay that verified the exact candidate stays Complete + records the SHA', () => {
  const normalized = normalizeAgentFinishForRole('verifier', { ...cleanInput, notes: notesWithBase(CANDIDATE) }, context(CANDIDATE))
...
test('Assay with a clean summary but no candidate fails closed to Hold', () => {
  for (const role of ['reviewer', 'verifier']) {
    const normalized = normalizeAgentFinishForRole(role, { ...cleanInput, notes: notesWithBase(MAIN_BASE, 'main') }, context(null))
...
test('Assay with clean summary but no workspace evidence fails closed to Hold', () => {
  const normalized = normalizeAgentFinishForRole('verifier', { ...cleanInput, notes: 'Assay ran but recorded no workspace line' }, context(CANDIDATE))
  assert.equal(normalized.resultStatus, 'Hold')
...
test('verifier failed/missing command evidence is never Complete (no context either)', () => {
  for (const testsSummary of ['1 failed', 'slack-notifier.test.ts does not exist in this checkout', 'No test files found']) {
    const normalized = normalizeAgentFinishForRole('verifier', { ...cleanInput, testsSummary })
...
test('builder finish behavior is unchanged by the Assay invariants', () => {
  const builderInput = { resultStatus: 'Complete', completion: 100, notes: 'smith done', commitHash: CANDIDATE, testsSummary: 'all checks passed' }
...
test('builder finish WITH a candidate hands off to mandatory QA carrying the candidate', async () => {
  const enqueued: Array<{ role: string; specialInstructions: string | null }> = []
  const followed = await followFinishedLane({
    storyId: 'ENG-FORGE-V4-10C-WITH-CANDIDATE', finishedRole: 'builder', resultStatus: 'Complete', candidateSha: CANDIDATE,
    getStory: async () => packetStory,
    enqueue: async (input) => { enqueued.push({ role: input.role, specialInstructions: input.specialInstructions ?? null }) },
...
test('builder finish WITHOUT a candidate never launches QA or Assay', async () => {
  ...
  const followed = await followFinishedLane({
    storyId: 'ENG-FORGE-V4-10C-NO-CANDIDATE', finishedRole: 'builder', resultStatus: 'Complete', candidateSha: null,
    getStory: async () => packetStory, enqueue: async (input) => { enqueued.push(input) }, repoRoot: '/definitely/missing',
  })
...
test('publish only runs when the Assay verified the exact candidate being published', async () => {
  let invoked = 0
  const publishSpy = async (input: PublishAcceptedCandidateInput): Promise<PublishAcceptedCandidateOutcome> => {
    invoked += 1
    const sha = input.candidateCommit ?? ''
    return { outcome: 'published', candidateCommit: sha, publishedMainHash: sha }
  }
  const report = await publishAcceptedCandidateAfterAssay({ role: 'verifier', resultStatus: 'Complete', testsSummary: 'all checks passed', candidateCommit: CANDIDATE, assayedCandidate: CANDIDATE, repoRoot: '/definitely/missing', publish: publishSpy })
...
test('publish refuses when the Assay verified a different base (V4-11 main case)', async () => {
  let invoked = 0
  const publishSpy = async () => { invoked += 1; return { outcome: 'no-candidate' as const, reason: 'unexpected invocation' } }
  for (const assayedCandidate of [MAIN_BASE, null, '', '   ']) {
    const report = await publishAcceptedCandidateAfterAssay({ role: 'verifier', resultStatus: 'Complete', testsSummary: 'all checks passed', candidateCommit: CANDIDATE, assayedCandidate, repoRoot: '/definitely/missing', publish: publishSpy })
...
test('omitting assayedCandidate keeps the legacy V4-10B publish decision unchanged', async () => {
  ...
  const report = await publishAcceptedCandidateAfterAssay({
    role: 'reviewer', resultStatus: 'Complete', testsSummary: 'accepted-candidate-publish.test.ts 4/4 pass', candidateCommit: CANDIDATE, repoRoot: '/definitely/missing',
    publish: async (input: PublishAcceptedCandidateInput): Promise<PublishAcceptedCandidateOutcome> => {
      invoked += 1
      const sha = input.candidateCommit ?? ''
      return { outcome: 'published', candidateCommit: sha, publishedMainHash: sha }
    },
