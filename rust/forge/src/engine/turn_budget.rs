...
pub fn resolve_generation_turn_cap(raw: Option<&str>) -> u32 {
    ...
    v.parse::<u32>().ok().map(|n| n.clamp(1, 100)).unwrap_or(DEFAULT_MAX_GENERATION_TURNS)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TurnBudgetVerdict {
    Allowed { turns_used: u32, cap: u32 },
    Refused { turns_used: u32, cap: u32, reason: String },
}
...
pub fn render_turn_budget_line(v: &TurnBudgetVerdict) -> String {
    let (used, cap, extra) = match v {
        ...
        TurnBudgetVerdict::Refused { turns_used, cap, reason } => (*turns_used, *cap, Some(reason.as_str())),
    };
    let remaining = cap.saturating_sub(used);
    let spent = format!("TURN BUDGET: {used} of {cap} model turns used; {remaining} before the cap");
    match extra {
        None => format!("{spent}."),
