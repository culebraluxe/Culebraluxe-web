...
pub const SERIAL_SMITH_NODES: &[&str] =
    &["smith", "repair_smith", "fast_smith", "fast_repair_smith"];
...
pub fn serial_launch_door(node_id: &str, has_accepted_assignment: bool) -> Option<String> {
    if !SERIAL_SMITH_NODES.contains(&node_id) {
        return None;
    }
...
pub fn serial_scope_miss_reasons(violations: &[String], owner: &str) -> Vec<String> {
    violations
        .iter()
        .map(|p| format!("{SCOPE_MISS_PREFIX}{p} is outside {owner}"))
        .collect()
}
