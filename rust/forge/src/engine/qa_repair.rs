//! Port of `workflow_app/forge/qa-repair-policy.ts`. Pure. No DB.
...
pub fn stored_reading_to_qa_result(
    reading: Option<QaDispositionReading>,
) -> (QaVerdict, Option<QaDisposition>) {
    match reading {
...
pub fn route_qa_result(
    ...
    if verdict == QaVerdict::Pass {
        return RepairRouting::Pass;
    }
    if verification_gap {
        return RepairRouting::Hold {
            reason: "VERIFICATION GAP: QA could not form/run a valid assay command plan (missing ## Assay commands or no frozen plan). Repair cannot help; fix the packet/config, then re-dispatch.".into(),
        };
    }
    if no_progress {
        return RepairRouting::Hold {
            reason: "NO_PROGRESS: the same candidate SHA re-failed the same machine classification with no new candidate in between — do not auto-launch another repair cycle.".into(),
        };
    }
    match disposition {
        None => RepairRouting::Hold {
            reason: "QA FAIL without a legal disposition (got None) — engine cannot choose a safe repair route; operator/Lead review required.".into(),
        },
        Some(QaDisposition::Escalate) => RepairRouting::Hold {
            reason: "QA requested ESCALATE — operator/Lead intervention required.".into(),
        },
        Some(QaDisposition::Repair) => {
            if state.repair_attempts >= budget.max_repair_attempts {
                RepairRouting::Hold {
                    reason: format!(
                        "Repair budget exhausted ({}/{}) — operator/Lead required.",
                        state.repair_attempts, budget.max_repair_attempts
                    ),
                }
            } else {
                RepairRouting::Smith {
        ...
        Some(QaDisposition::Replan) => {
            if state.replan_attempts >= budget.max_replan_attempts {
                RepairRouting::Hold {
                    reason: format!(
                        "Replan budget exhausted ({}/{}) — operator/Lead required.",
                        state.replan_attempts, budget.max_replan_attempts
                    ),
                }
            } else {
