//! Forge command inventory. RE commands never live here.
...
pub const XML_RELEASE_COMMANDS: &[&str] = &[
    PUBLISH_CANDIDATE, MIGRATE_DEV, VERIFY_DEV_MIGRATION, MIGRATE_PROD,
    VERIFY_PROD_MIGRATION, REFRESH_DERIVED_MODELS, VERIFY_DERIVED_MODELS,
];

pub const ROUTED: &[&str] = &[
    STORY_MARK_HOLD, STORY_MARK_COMPLETE, STORY_MARK_IN_PROGRESS, RUN_APPEND_DETAIL,
    RUN_SCOUT, RUN_DIAGNOSIS, RUN_ARCHITECT, RUN_LEAD_PRE, RUN_LEAD_IMPLEMENT,
    RUN_SMITH, RUN_SMITH_SPLIT, RUN_LEAD_POST, RUN_QA_REVIEW, RUN_QA_VERIFY,
    CLASSIFY_FAILURE, RUN_SMITH_REPAIR, RUN_ARCHITECT_REPAIR, RUN_DEVOPS_REPAIR,
    PUBLISH_CANDIDATE, MIGRATE_DEV, VERIFY_DEV_MIGRATION, MIGRATE_PROD,
    VERIFY_PROD_MIGRATION, REFRESH_DERIVED_MODELS, VERIFY_DERIVED_MODELS,
    DEPLOY, VERIFY_PRODUCTION,
];

pub fn is_routed(command_type: &str) -> bool { ROUTED.contains(&command_type) }
pub fn is_release(command_type: &str) -> bool { XML_RELEASE_COMMANDS.contains(&command_type) }
pub fn is_forge_namespace(command_type: &str) -> bool { command_type.starts_with("forge.") }
