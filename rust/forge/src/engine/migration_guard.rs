...
pub fn assess_migration_applied(changed_paths: &[String], ledger: &[String]) -> Vec<String> {
    let files: Vec<String> = changed_paths.iter().filter_map(|p| migration_ledger_filename(p)).collect();
    let mut seen = std::collections::BTreeSet::new();
    let ledger: std::collections::BTreeSet<_> = ledger.iter().filter_map(|f| migration_ledger_filename(f)).collect();
    let mut unapplied = Vec::new();
    for f in files {
        if !seen.insert(f.clone()) { continue; }
        if !ledger.contains(&f) { unapplied.push(f); }
    }
    unapplied
}

pub fn migration_applied_refusal(unapplied: &[String]) -> String {
    format!("story change set adds migration(s) not recorded in the PROD schema_migration ledger: {}", unapplied.join(", "))
}
