...
pub fn classify_first_violation(o: &FirstViolationObservations) -> (FirstViolation, Vec<String>) {
    if o.acceptance_incomplete {
        return (
            FirstViolation::Underspecified,
            vec!["acceptance or ownership was incomplete before execution".into()],
        );
    }
    let mut because = Vec::new();
    if o.door_refused {
        because.push("a door refused the lane".into());
    }
    if o.repeated_candidate_failure {
        because.push("the same candidate re-failed without new work".into());
    }
    if o.evidence_write_failed {
        because.push("the control plane failed to record evidence".into());
    }
    for r in &o.reasons {
        because.push(format!("observed: {r}"));
    }
    if !because.is_empty() {
        ...
        (
            ...
            vec!["no door, candidate repeat or acceptance gap was observed".into()],
        )
    }
}
...
pub fn render_first_violation(v: FirstViolation, because: &[String]) -> String {
    ...
    let detail = if because.is_empty() {
        "no evidence recorded".into()
    } else {
        because.join("; ")
    };
