...
impl ForgeCommandRegistry {
    pub fn build(
        ...
        }
        {
            let w = writer.clone();
            handlers.insert(
                commands::RUN_APPEND_DETAIL.into(),
                Box::new(move |req| {
                ...
                handlers.insert(
                    ...
                    Box::new(move |req| {
                        cmd_result(
                            ...
                            ApplicationCommandOutcome::PreconditionFailure,
                            Some(format!(
                                "{name} is not executable as a command; use a claimed Forge engine task"
                            )),
                        )
                    }),
                );
    ...
    pub fn dispatch(&self, req: &ApplicationCommandRequest) -> ApplicationCommandResult {
        if !commands::is_routed(&req.command_type) {
            return cmd_result(
                ...
                Some(format!(
                    "Not a Forge command: {} (Forge executes forge.* commands only)",
                    req.command_type
                )),
            );
        }
        match self.handlers.get(&req.command_type) {
            ...
            None => cmd_result(
                &req.command_id,
                ApplicationCommandOutcome::NotFound,
                Some(format!("Unknown Forge command: {}", req.command_type)),
            ),
        }
    }
}
