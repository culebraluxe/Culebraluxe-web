...
pub enum MarkerVerdict { Passed, Failed, Skipped }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AbsentReason { NotRun, WrongFile, NoProvenance, Ambiguous }
...
pub enum AssertionResolution {
    ...
    Failed,
    Skipped,
    Absent { reason: AbsentReason, detail: String },
}
...
pub fn parse_assertion_ref(r#ref: &str) -> (Option<String>, String) {
    ...
    match trimmed.rfind('#') {
        Some(at) => {
            ...
            (if file.is_empty() { None } else { Some(file.to_string()) }, name.to_string())
        }
...
fn marker_name(line: &str) -> Option<String> {
    ...
    let raw = if let Some(rest) = text.strip_prefix("not ok").or_else(|| {
        text.strip_prefix("ok").filter(|s| s.starts_with(|c: char| c.is_whitespace() || c == '-'))
    }) {
        let rest = rest.trim_start();
        let rest = rest.trim_start_matches(|c: char| c.is_ascii_digit()).trim_start();
        rest.strip_prefix('-').unwrap_or(rest).trim()
    } else if text.starts_with(['✔', '✓', '✖', '✗', '✘']) {
        text.chars().next().and_then(|_| Some(text.chars().skip(1).collect::<String>()))
            .as_deref().map(str::trim).unwrap_or("")
    } else {
        return None;
    };
    let cleaned = raw
...
fn junit_entries(text: &str) -> Vec<AssertionEntry> {
    ...
    while let Some(at) = rest.find("<testcase") {
        rest = &rest[at + 9..];
        let Some(end_attrs) = rest.find('>') else { break };
        ...
        if let Some(name) = attr(attrs.trim_end_matches('/'), "name") {
            ...
            entries.push(AssertionEntry { name: decode_xml(name), file, verdict });
        }
...
fn tap_entries(text: &str) -> Vec<AssertionEntry> {
    ...
    for (i, line) in lines.iter().enumerate() {
        let Some(verdict) = marker_line(line) else { continue };
        ...
        if lines.get(i + 1).map(|s| s.trim()) == Some("---") {
            for follow in lines.iter().skip(i + 2) {
                ...
                if let Some(loc) = follow.trim().strip_prefix("location:") {
                    ...
                    if !without.is_empty() { file = Some(without.to_string()); }
                    break;
                }
            }
        }
        entries.push(AssertionEntry { name, file, verdict });
    }
    entries
}

fn same_file_path(candidate: &str, wanted: &str) -> bool {
    let clean = |v: &str| v.replace('\\', "/").trim_start_matches("./").trim_end_matches('/').to_string();
...
pub fn assertion_resolution(output: &str, r#ref: &str) -> AssertionResolution {
    ...
    if name.is_empty() {
        return AssertionResolution::Absent { reason: AbsentReason::NotRun, detail: "the ref names no assertion".into() };
    }
    ...
    if let Some(wanted) = file {
        let matched: Vec<_> = by_name.iter().filter(|e| e.file.as_deref().map(|f| same_file_path(f, &wanted)).unwrap_or(false)).cloned().collect();
        if matched.is_empty() {
...
fn fold(entries: &[AssertionEntry]) -> AssertionResolution {
    if entries.iter().any(|e| e.verdict == MarkerVerdict::Failed) { return AssertionResolution::Failed; }
    if entries.iter().any(|e| e.verdict == MarkerVerdict::Passed) { return AssertionResolution::Passed; }
...
mod tests {
    ...
    fn skip_is_not_pass() {
        ...
        assert_eq!(assertion_resolution(out, "required assertion"), AssertionResolution::Skipped);
    }
    #[test]
    fn file_qualified_ref_rejects_other_file() {
        ...
        match assertion_resolution(out, "a.test.ts#same assertion") {
            AssertionResolution::Absent { reason: AbsentReason::WrongFile, .. } => {}
            other => panic!("{other:?}"),
        }
    }
}
