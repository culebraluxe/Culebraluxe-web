    ...
    pub async fn command(&mut self, request: TechCommandRequest, context: &ServiceContext)
        ...
        let result:Result<TechCommandResult,CoreServiceError>=async {
            ...
            match request.action.as_str() {
              ...
              "goodToGo" => {
                if id.is_empty(){return Err(CoreServiceError::Validation("Missing story id.".into()))}
                let active=self.repository.active_agent_work(id).await?;
                if let Some(w)=active.first(){return Err(CoreServiceError::Conflict(format!("{id} is already with Forge ({}).",w["state"].as_str().unwrap_or("active"))))}
                self.repository.set_active_work(id,false,actor).await?;
                self.repository.story_status(id,"Ready").await?;
                if self.repository.set_dispatch_options(id,None).await?==0{return Err(CoreServiceError::Conflict("No queued work item to hand off — this story is already running.".into()))}
              ...
              "scopedRun" => {
                ...
                if !matches!(stop,"scout"|"architect"|"lead"){return Err(CoreServiceError::Validation(format!("Unsupported scoped stop: {stop}")))}
                let active=self.repository.active_agent_work(id).await?;
                if let Some(w)=active.iter().find(|w|matches!(w["state"].as_str(),Some("Claimed"|"Running"|"Paused"))){return Err(CoreServiceError::Conflict(format!("{id} is already executing ({}); a live run cannot be re-scoped.",w["state"].as_str().unwrap_or("active"))))}
                if !active.iter().any(|w|w["state"]=="Ready"){self.repository.story_status(id,"Ready").await?}
                if self.repository.set_dispatch_options(id,Some(stop)).await?==0{return Err(CoreServiceError::Conflict("No queued work item to scope — this story is already running.".into()))}
                ok(format!("{id} queued through {stop}; it remains on the Workbench for review."))
              }
              "moveWorkbench" => {
                if id.is_empty(){return Err(CoreServiceError::Validation("Missing story id.".into()))}
                let target=request.target.as_deref().unwrap_or("");
                let status=match target{"backlog"=>"Backlog","closed"=>"Closed","next"=>"Next Version",_=>return Err(CoreServiceError::Validation(format!("Unsupported Workbench destination: {target}")))};
              ...
              "cancelFlight" => {
                ...
                if batch.is_empty(){return Err(CoreServiceError::Validation("Missing Flight id.".into()))}
                if self.repository.cancel_batch(batch).await?==0{return Err(CoreServiceError::Conflict("That Flight is not waiting to fire.".into()))}
                ok("Scheduled Flight cancelled. Nothing was dispatched.".into())
              }
              "launchFlight"|"scheduleFlight" => Err(CoreServiceError::Validation("Flight dispatch is owned by the Rust Forge engine and is not available through this compatibility command.".into())),
              other=>Err(CoreServiceError::Validation(format!("Unknown TECH Cockpit command: {other}")))
            }
        }.await;
        audit_result(&self.runtime,"tech",OP,context,decision,&result).await?;
        result
