...
async fn replace_mappings(
    ...
    sqlx::query("delete from contract_property where contract_id = $1::uuid")
        .bind(&request.contract_id)
        .execute(tx.connection())
        .await
        .map_err(|error| {
            DbFailure::from_sqlx("contract.replace_mappings.delete_property", &error)
        })?;
    sqlx::query("delete from contract_person where contract_id = $1::uuid")
        .bind(&request.contract_id)
        .execute(tx.connection())
        .await
        .map_err(|error| {
