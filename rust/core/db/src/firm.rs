...
use domain::{FieldPatch, Firm, UpsertFirmRequest};
use sqlx::FromRow;
...
impl FirmDao {
    ...
    pub async fn find_by_name(&self, name: &str) -> DbResult<Option<Firm>> {
        ...
        let rows = sqlx::query_as::<_, FirmRow>(
            ...
            order by case
                       when lower(trim(coalesce(legal_name, ''))) = lower(trim($1)) then 0
                       else 1
                     end,
                     id
            limit 2
    ...
    pub async fn upsert(&self, request: &UpsertFirmRequest) -> DbResult<Firm> {
        ...
        let legal_name = apply_patch(
            &request.legal_name,
            existing.as_ref().and_then(|firm| firm.legal_name.clone()),
        );
        let kind = apply_patch(
            &request.kind,
            existing.as_ref().and_then(|firm| firm.kind.clone()),
        );
        let status = request
            .status
            .as_deref()
