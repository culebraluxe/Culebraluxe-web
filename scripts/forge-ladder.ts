...
const LADDER: LadderRung[] = [
  {
    ...
    acceptance:
      'pnpm forge:doctor prints instances, open tasks, open work items and active engine claims, plus the age of the oldest active claim, and writes nothing: no update, no insert, no claim. It also reports whether the scheduled WORKER is alive, from its own logs: the newest invocation (time, outcome) and the most recent failure reason, read from AGENT_WORKER_LOG_DIR (default ~/Library/Logs/CulebraLuxe). It also renders the POSTCARD block the Grok<->DeepSeek mailbox asks for - board-vs-table agreement, active decision count, the 7-day ROI rows, the newest learn-pass attempt and the worker liveness line - so a reply is one command instead of five hand-pasted ones. Rendering is a pure function with a unit test, including the empty-control-plane case, the board-drifted-from-table case and the worker-failing case.',
    notes:
