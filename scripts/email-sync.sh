...
log "verifying metadata-only Apple Mail environment"
...
const required=["DATABASE_URL_DEV","DATABASE_URL_PROD","ICLOUD_MAIL_ADDRESS","ICLOUD_MAIL_USERNAME","ICLOUD_MAIL_APP_PASSWORD","EMAIL_INTERNAL_ADDRESSES"];
...
if(values.get("ICLOUD_MAIL_APP_PASSWORD").includes("\"")||values.get("ICLOUD_MAIL_APP_PASSWORD").includes("\x27")){console.error("ICLOUD_MAIL_APP_PASSWORD must not include quote characters");process.exit(2)}
