...
import { openForgeHoldRecord } from '@/legacy/db/forge-hold'
import { markForgeStoryHumanHold } from '@/legacy/db/forge-story-state'
...
import {
  ...
  lessonsForArea,
} from '@/legacy/workflow_app/forge/forge-context-lessons'
import {
...
import { getAgentWorkItem, listActiveAgentWorkItems } from '@/legacy/db/agent-work'
import {
...
import { readForgeRepairLedger } from '@/legacy/db/forge-repair-ledger'
import { readForgeWorkflowEvidence, mergeForgeWorkflowEvidence } from '@/legacy/db/forge-workflow-evidence'
import {
  listSplitChildOutcomes,
  recordSplitChildAssignment,
  recordSplitChildCandidate,
} from '@/legacy/db/forge-split-children'
import { splitJoinHoldReasons } from '@/legacy/workflow_app/forge/split-join'
import { getStoryboardStory, listStoryCommitHashes, listStoryRunBaseCommits, setStoryArchitectBrief, setStoryScoutPacket } from '@/legacy/db/storyboard'
import { assertForgeExecutionTarget, assertForgeLaneMayStart } from '@/legacy/workflow_app/forge/forge-execution-target'
import { assessSmithWork, smithDispatchRunDetail } from '@/legacy/workflow_app/forge/forge-dispatch-seam'
import { assessArchitectBrief, shapeArchitectFindings } from '@/legacy/workflow_app/forge/forge-shaping'
import { renderSmithWorkOrders } from '@/legacy/workflow_app/forge/forge-lead-plan'
import { leadRoutingFacts } from '@/legacy/workflow_app/forge/forge-lead-routing'
import { buildLeadRoutingDirective } from '@/legacy/workflow_app/forge/forge-lead-routing-prompt'
import { candidateOwnChangedFiles, recordedScopeBase, storyScopeBase } from '@/legacy/workflow_app/forge/story-scope-base'
import type { RoleEffectPorts } from '@/legacy/workflow_app/forge/agents/ports'
import { existsOnGitBaseRef, commitOnGitBaseRef } from '@/legacy/workflow_app/forge/agents/architect/exists-git'
import { CANDIDATE_SHA, describeRefusal, mediateField } from '@/lib/field-mediator'
import { parseArchitectHandoff } from '@/legacy/workflow_app/forge/agents/architect-handoff'
import { seamGroupHint } from '@/legacy/workflow_app/forge/agents/architect/shape-hint'
import { smithWorkOrdersFromFindings } from '@/legacy/workflow_app/forge/agents/architect/persist'
import { assignmentFromLead } from '@/legacy/workflow_app/forge/agents/smith/from-lead'
import { negativeControlForStory } from '@/legacy/workflow_app/forge/agents/negative-control-supplier'
import { buildSmithDirective } from '@/legacy/workflow_app/forge/agents/smith/prompt'
import { buildSelfHealDirectiveWithReasons } from '@/legacy/workflow_app/forge/agents/self-heal'
import {
  getForgeRoleContract,
  getLatestForgeRoleContractForStory,
  type ForgeRoleContract,
} from '@/legacy/db/forge-role-contract'
import { getForgeRolePlan, type ForgeRolePlan } from '@/legacy/db/forge-role-plan'
import { listStoryForgeFindingHandoff } from '@/legacy/db/forge-role-finding'
import type { LeadAssignment, RoutingReview } from '@/legacy/workflow_app/forge/forge-lead-routing'
import { resolveLeadProposal } from '@/legacy/workflow_app/forge/lead-proposal-resolve'
import { buildArchitectDirective } from '@/legacy/workflow_app/forge/forge-architect-directive'
import { architectContractFromNotes, resolveAcceptanceAssertions } from '@/legacy/workflow_app/forge/forge-architect-contract'
import { acceptanceClauses, buildAcceptanceMap } from '@/legacy/workflow_app/forge/agents/qa/types'
import { assessBaselineAcceptance } from '@/legacy/workflow_app/forge/baseline-acceptance'
import { classifyFirstViolation, renderFirstViolation } from '@/legacy/workflow_app/forge/first-violation'
...
} from '@/legacy/workflow_app/forge/model-turn-budget'
import { describeClaimBlocker } from '@/legacy/workflow_app/forge/forge-claim-blocker'
import { DEFAULT_FORGE_STALE_MS, recoverStaleForgeEngineClaims } from '@/legacy/db/forge-engine-recovery'
import { scorePlanDetailed } from '@/legacy/workflow_app/forge/forge-plan-difficulty'
import { SCORER_ID } from '@/legacy/workflow_app/forge/forge-difficulty-scorer'
import {
  recordForgeDispatchOutcome,
  recordForgeDispatchScore,
} from '@/legacy/db/forge-dispatch-score'
import { assessSmithExit } from '@/legacy/workflow_app/forge/smith-candidate'
import { commandRunner, staticSliceForWorktree } from '@/legacy/workflow_app/forge/agents/exec-command'
import { renderSplitAssignmentWorkOrders, smithContractFromAssignment, splitChildAssignment } from '@/legacy/workflow_app/forge/forge-split-handoff'
import type { SmithExecutionContract } from '@/legacy/workflow_app/forge/smith-contract'
import { createPersistentTraceSink } from '@/legacy/workflow_app/forge/forge-observer'
import {
  drainAlerts,
  observeAttemptBegin,
  observeAttemptEnd,
...
import { serialLaunchDoor, serialScopeMissReasons } from '@/legacy/workflow_app/forge/forge-serial-doors'
import { recordTraceEvent, listTraceEvents } from '@/legacy/db/workflow-trace'
import { changedFilesForCandidate } from '@/lib/worker-workspace/candidate-diff'
import { guardMigrationApplied, migrationAppliedRefusal } from '@/legacy/workflow_app/forge/migration-applied-guard'
import {
  acceptedLeadRouting,
  buildLeadRoutingContext,
  findLatestAcceptedLeadRouting,
  type LeadRoutingCapabilities,
} from '@/legacy/workflow_app/forge/lead-routing-context'
...
import type { ForgeRoleRunner } from '@/legacy/workflow_app/forge/forge-executor'
import {
  buildSelfHealDirective,
  deliverableEnforcementEnabled,
  parseDeliverableRepromptBudget,
  rawRoleOutput,
} from '@/legacy/workflow_app/forge/agents/forge-phase-agent'
import { forgeAgentFor } from '@/legacy/workflow_app/forge/agents/role-agents'
import {
...
} from '@/legacy/workflow_app/forge/forge-typed-evidence'
import {
  attestIntegration,
  gitIsAncestor,
...
import { forgeDeployHoldReason } from '@/legacy/workflow_app/forge/forge-facts'
...
export function createAgentRuntimeForgeRoleRunner(
  ...
  return async (nodeId, task) => {
    ...
    if ((nodeId === 'qa_verify' || nodeId === 'fast_qa_verify') && evidence.qaPassed === false) {
      ...
      try {
        const { readStoryForgeConvergence } = await import('@/legacy/db/forge-convergence')
