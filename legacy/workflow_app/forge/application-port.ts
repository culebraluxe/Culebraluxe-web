...
import { FORGE_ROUTED_COMMAND_TYPES } from '@/legacy/workflow_app/forge-command-types'
import { buildForgeCommandRegistry, dispatchForgeCommand } from '@/legacy/workflow_app/forge/forge-command'
import type {
  ForgeFactReader,
  ForgeStateWriter,
  ForgeReleaseExecutor,
} from '@/legacy/workflow_app/forge/forge-state-writer'
...
async function resolveWriter(
  ...
  if (!opts.writer) {
    const { createDbForgeStateWriter } = await import('@/legacy/workflow_app/forge/db-state-writer')
  ...
  return {
    ...
    async readFacts(subject: WorkflowSubject): Promise<ApplicationFacts> {
      ...
      if (subject.subjectType === 'story') {
        const reader =
          opts.evidenceReader ??
          (await import('@/legacy/workflow_app/forge/forge-evidence-db')).createStoryGateEvidenceReader()
