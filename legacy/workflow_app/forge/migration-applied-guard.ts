...
// is the repository-relative path the migration command recorded — `legacy/db/migrations/<name>.sql`
...
export function migrationLedgerFilename(path: string): string | null {
  ...
  return /^legacy\/db\/migrations\/[^/]+\.sql$/i.test(normalized) ? normalized : null
}
...
export function migrationsAddedSince(sinceIso: string, repoRoot: string = process.cwd()): string[] {
  const out = execFileSync(
    'git',
    ['log', '--diff-filter=A', '--name-only', '--format=', `--since=${sinceIso}`, '--', 'legacy/db/migrations'],
