...
test('parses one SMITH_CANDIDATE line', () => {
  const notes = `${SMITH_CANDIDATE_PREFIX} {"version":1,"assignmentId":"a1","candidateSha":"aaaaaaaa","mergeBase":"bbbbbbbb","changedPaths":["legacy/workflow_app/forge/a.ts"]}`
  const c = parseSmithCandidate(notes)
  assert.equal(c?.assignmentId, 'a1')
  assert.equal(c?.candidateSha, 'aaaaaaaa')
  assert.deepEqual(c?.changedPaths, ['legacy/workflow_app/forge/a.ts'])
})
