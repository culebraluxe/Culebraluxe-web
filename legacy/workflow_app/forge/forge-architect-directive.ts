...
export function buildArchitectDirective(baseRef: string, frozenProofs: string[]): string {
  ...
  return [
    ...
    'NEW FILE? Declare its PARENT DIRECTORY as a scope entry (e.g. "legacy/workflow_app/tests/"). A seam must EXIST on baseRef, and a directory does — while a file that is not there yet does not. Without the directory the Lead has no legal surface to assign and the story HOLDs even though your plan is sound. This is the law in `seamForNewFile` (workflow_app/forge/forge-shaping.ts): the function wins if this sentence and it ever drift.',
