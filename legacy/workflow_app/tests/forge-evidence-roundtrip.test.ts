...
} from '@/legacy/db/forge-workflow-evidence'
...
test(
  ...
  const full: ForgeGateEvidence = {
    ...
    migrationFiles: ['legacy/db/migrations/137_fixture.sql', 'legacy/db/migrations/138_fixture.sql'],
    ...
    findings: [
      {
        ...
        summary: 'forge seam',
        required: true,
        seams: ['legacy/workflow_app/forge/'],
        hint: 'SAME_UNIT',
      },
      {
        id: 'adj',
  ...
  try {
    ...
    assert.deepEqual(got.migrationFiles, [
      'legacy/db/migrations/137_fixture.sql',
      'legacy/db/migrations/138_fixture.sql',
    ])
    ...
    assert.deepEqual(got.findings?.[0].seams, ['legacy/workflow_app/forge/'])
