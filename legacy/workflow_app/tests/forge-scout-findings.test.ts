...
test('scout-style FORGE_FINDINGS_JSON marker parses into findings', () => {
  const notes =
    ...
    'FORGE_FINDINGS_JSON: [{"id":"scout-1","summary":"forge-executor owns the stopAfter park logic","required":false,"seams":["legacy/workflow_app/forge/forge-executor.ts"],"hint":"NOTE"}]\n'
  ...
  assert.deepEqual(findings[0]!.seams, ['legacy/workflow_app/forge/forge-executor.ts'])
})
