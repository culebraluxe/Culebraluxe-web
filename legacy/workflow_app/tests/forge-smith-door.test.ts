...
} from '@/legacy/workflow_app/forge/forge-serial-doors'
import { createAgentRuntimeForgeRoleRunner } from '@/legacy/workflow_app/forge/agent-runtime-role-runner'
...
test('wiring: the four Smith roles are the ones the role mapping actually emits', async () => {
  ...
  const { forgeRoleNodePlan } = await import('@/legacy/workflow_app/forge/forge-role-mapping')
