...
const MIGRATION = readFileSync('legacy/db/migrations/096_integration_source_snapshot_member.sql', 'utf8')
...
test('lifecycle G: the single promotion is the only thing that reads the landing tables', () => {
  const promote = readFileSync('legacy/db/promote-evidence.ts', 'utf8')
