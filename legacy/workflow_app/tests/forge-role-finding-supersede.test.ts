...
import { decideFindingWrite, listStoryForgeFindingHandoff } from '@/legacy/db/forge-role-finding'
...
const ATTEMPT_ONE = {
  attempt: 1,
  findings: [
    { findingId: 'F1', seams: ['legacy/db/forge-role-finding.ts'] },
...
test('a later attempt that drops a seam is refused, naming the seam and the attempt that declared it', () => {
  const decision = decideFindingWrite({
    ...
    prior: ATTEMPT_ONE,
    current: [],
    incoming: { findingId: 'F1', seams: ['legacy/db/forge-role-finding.ts'] },
    acknowledged: [],
  })
...
test('a re-issue that preserves every seam is allowed, even when it re-keys the finding', () => {
  const decision = decideFindingWrite({
    ...
    incoming: {
      findingId: 'RENAMED',
      seams: ['legacy/db/forge-role-finding.ts', 'scripts/forge-handoff.mjs'],
    },
    acknowledged: [],
  })
...
const findingRow = (over: QueryRow = {}): QueryRow => ({
  ...
  required: true,
  seams: ['legacy/db/forge-role-finding.ts'],
  hint: null,
  attempt: 2,
  ...over,
})
...
const FINDINGS = [{ id: 'F1', required: true, seams: ['legacy/db/forge-role-finding.ts'] }]
