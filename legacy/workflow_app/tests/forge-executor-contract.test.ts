...
import { driveForgeStory, planWave, type WaveLane } from '@/legacy/workflow_app/forge/forge-executor'
import { forgeLaneSurface } from '@/legacy/workflow_app/forge/agent-runtime-role-runner'
import { shapeArchitectFindings, shapeSizeFloor } from '@/legacy/workflow_app/forge/forge-shaping'
import { reviewLeadProposal } from '@/legacy/workflow_app/forge/forge-lead-routing'
import {
...
test('ENG-FORGE-PARALLEL-WAVE-01: two non-split ready lanes share a wave under the cap', () => {
  const plan = planWave(
    [lane('smith', ['legacy/workflow_app/a.ts']), lane('qa_verify', ['legacy/workflow_app/b.ts'])],
    2,
  )
...
test('ENG-FORGE-PARALLEL-WAVE-01: a one-task wave runs exactly once', () => {
  const plan = planWave([lane('smith', ['legacy/workflow_app/a.ts'])], 2)
  assert.equal(plan.ok, true)
  if (!plan.ok) return
  assert.equal(plan.batches.length, 1)
...
test('ENG-FORGE-CONTRACT-ONE-WRITER-01: an overlapping pair is deferred and a disjoint lane still runs', () => {
  const plan = planWave(
    [
      lane('smith', ['legacy/workflow_app/a.ts']),
      lane('qa_verify', ['legacy/workflow_app/a.ts']),
      lane('inspector', ['legacy/workflow_app/c.ts']),
    ],
    2,
  )
  assert.equal(plan.ok, true)
  ...
  assert.equal(plan.refusals[0].path, 'legacy/workflow_app/a.ts')
})

test('ENG-FORGE-CONTRACT-ONE-WRITER-01: a lone overlapping pair still progresses in separate batches', () => {
  const plan = planWave(
    [lane('smith', ['legacy/workflow_app/a.ts']), lane('qa_verify', ['legacy/workflow_app/a.ts'])],
    2,
  )
  assert.equal(plan.ok, true)
  assert.deepEqual(
  ...
  assert.deepEqual(plan.refusals, [{ lanes: ['smith', 'qa_verify'], path: 'legacy/workflow_app/a.ts' }])
})

test('ENG-FORGE-PARALLEL-WAVE-01: an undeclared surface never runs concurrently', () => {
  const plan = planWave([lane('smith', null), lane('qa_verify', ['legacy/workflow_app/b.ts'])], 2)
  assert.equal(plan.ok, true)
  if (!plan.ok) return
  assert.deepEqual(
...
test('ENG-FORGE-SURFACE-SUPPLIER-01: the supplier feeds planWave so two non-fanout lanes with disjoint surfaces share one wave', () => {
  const a = forgeLaneSurface({ formData: { surface: ['legacy/workflow_app/a.ts'] } })
  const b = forgeLaneSurface({ formData: { surface: ['legacy/workflow_app/b.ts'] } })
  assert.deepEqual(a, ['legacy/workflow_app/a.ts'])
  assert.deepEqual(b, ['legacy/workflow_app/b.ts'])
...
test('ENG-FORGE-SURFACE-SUPPLIER-01: a lane with no declared surface still runs alone', () => {
  ...
  const plan = planWave([lane('smith', undeclared), lane('qa_verify', ['legacy/workflow_app/b.ts'])], 2)
  assert.equal(plan.ok, true)
  if (!plan.ok) return
  assert.deepEqual(
    plan.batches.map((batch) => batch.length),
...
test('ENG-FORGE-SURFACE-SUPPLIER-01: the declared surface round-trips through the commit marker', () => {
  const surface = ['lib/worker-workspace/commit.ts', 'legacy/workflow_app/forge/forge-shaping.ts']
...
test('ENG-FORGE-SURFACE-SUPPLIER-01: the MEDIUM floor comes from the shaper seam groups, not the model rating', () => {
  const oneGroup = shapeArchitectFindings({
    findings: [
      { id: 'a', summary: 'a', required: true, seams: ['legacy/workflow_app/a.ts'] },
      { id: 'b', summary: 'b', required: true, seams: ['legacy/workflow_app/a.ts'] },
    ],
  })
  ...
  const twoGroups = shapeArchitectFindings({
    findings: [
      { id: 'a', summary: 'a', required: true, seams: ['legacy/workflow_app/a.ts'] },
      { id: 'b', summary: 'b', required: true, seams: ['legacy/workflow_app/b.ts'] },
      { id: 'c', summary: 'c', required: false, seams: ['legacy/workflow_app/c.ts'] },
    ],
  })
...
test('ENG-FORGE-SURFACE-SUPPLIER-01: a SMALL plan over two shaper seam groups is refused', () => {
  const review = reviewLeadProposal(
            ...
            chunks: [
              {
                id: 1,
                outcome: 'o',
                surface: ['legacy/workflow_app/a.ts'],
                invariant: 'i',
                proof: FROZEN_PROOF,
                dependsOn: [],
              },
    ...
    {
      findings: [
        { id: 'a', required: true, seams: ['legacy/workflow_app/a.ts'] },
        { id: 'b', required: true, seams: ['legacy/workflow_app/b.ts'] },
      ],
      evidenceRefs: ['story_goal'],
      splitEnabled: true,
      maxSmiths: 2,
