...
const contract = (over: Partial<SmithExecutionContract> = {}): SmithExecutionContract => ({
  ...
  requiredInputs: [],
  allowedScope: ['legacy/workflow_app/forge/a.ts', 'legacy/workflow_app/tests/a.test.ts'],
  prohibitedScope: ['legacy/workflow_app/forge/b.ts', 'legacy/workflow_app/tests/b.test.ts'],
  expectedOutputs: [],
  requiredEvidence: [],
  dependsOn: [],
  ...over,
...
test('a change inside the assignment is clean', () => {
  assert.deepEqual(
    scopeViolations(contract(), ['legacy/workflow_app/forge/a.ts', 'legacy/workflow_app/tests/a.test.ts']),
    [],
  )
})

test("a sibling's file is a violation even when it otherwise looks fine", () => {
  const violations = scopeViolations(contract(), ['legacy/workflow_app/forge/a.ts', 'legacy/workflow_app/forge/b.ts'])
  assert.deepEqual(violations, ['legacy/workflow_app/forge/b.ts'])
})
...
test('symbol-level scope entries accept the whole FILE (git reports files, not symbols)', () => {
  ...
  const c = contract({
    allowedScope: ['legacy/workflow_app/forge/a.ts#doTheThing', 'legacy/workflow_app/tests/a.test.ts#case'],
    prohibitedScope: ['legacy/workflow_app/forge/b.ts#otherThing'],
  })
  assert.deepEqual(scopeViolations(c, ['legacy/workflow_app/forge/a.ts', 'legacy/workflow_app/tests/a.test.ts']), [])
})

test("a sibling's symbol-level surface still forbids the whole sibling file", () => {
  const c = contract({
    allowedScope: ['legacy/workflow_app/forge/a.ts#doTheThing'],
    prohibitedScope: ['legacy/workflow_app/forge/b.ts#otherThing'],
  })
  assert.deepEqual(scopeViolations(c, ['legacy/workflow_app/forge/b.ts']), ['legacy/workflow_app/forge/b.ts'])
})
...
test('changedFilesForCandidate reports the child diff from its merge base', async () => {
  ...
  assert.deepEqual(changed, ['legacy/workflow_app/forge/a.ts'])
  ...
  assert.deepEqual(scopeViolations(contract({ allowedScope: ['legacy/workflow_app/forge/base.ts'], prohibitedScope: [] }), changed), [
