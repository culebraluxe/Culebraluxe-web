...
import { reviewLeadProposal, type LeadProposal, type RoutingContext } from '@/legacy/workflow_app/forge/forge-lead-routing'
...
function solo(): { p: LeadProposal; context: RoutingContext } {
  return {
    context: {
      findings: [{ id: 'behavior', required: true, seams: ['legacy/db/contact.ts'] }],
...
test('scope alias is accepted as surface', () => {
  ...
  chunk.scope = ['legacy/db/contact.ts']
  delete chunk.surface
  const r = reviewLeadProposal(p, context)
  assert.equal(r.ok, true)
  if (r.ok) assert.deepEqual(r.proposal.assignments[0].plan.chunks[0].surface, ['legacy/db/contact.ts'])
})
...
test('Bench SOLO refuses SPLIT', () => {
  ...
  p.decision = 'SPLIT'
  p.size = 'LARGE'
  context.findings.push({ id: 'other', required: true, seams: ['legacy/db/other.ts'] })
  const b = structuredClone(p.assignments[0])
  b.id = 'b'
  b.findingIds = ['other']
  b.plan.chunks[0].surface = ['legacy/db/other.ts']
