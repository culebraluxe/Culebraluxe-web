...
import { getClientRelationshipChannels } from '@/legacy/db/relationship-channels'
...
test('F: Client directory freshness uses relationship/source state as authoritative', () => {
  const mig = readFileSync('legacy/db/migrations/094_mv_client_relationship_channels.sql', 'utf8')
  ...
  const refresh = readFileSync('legacy/db/client-read-models.ts', 'utf8')
  const order = [
...
test('G: primary Client History panel renders six compact latest-activity source rows', () => {
  ...
  const correction = readFileSync('legacy/db/migrations/099_relationship_latest_context.sql', 'utf8')
  ...
  const unifiedEmail = readFileSync('legacy/db/migrations/100_unified_email_relationship_channel.sql', 'utf8')
  ...
  const repo = readFileSync('legacy/db/relationship-channels.ts', 'utf8')
