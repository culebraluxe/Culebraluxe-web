...
import { reviewLeadProposal, parseLeadRouting, leadRoutingFacts } from '@/legacy/workflow_app/forge/forge-lead-routing'
import type { LeadProposal, RoutingContext } from '@/legacy/workflow_app/forge/forge-lead-routing'
...
function fixture(): { p: LeadProposal; context: RoutingContext } {
  return {
    context: {
      findings: [{ id: 'behavior', required: true, seams: ['legacy/db/contact.ts', 'legacy/services/contact.ts', 'components/contact.tsx'] }],
    ...
    p: {
      ...
      assignments: [{
        ...
        plan: { size: 'SMALL', chunks: [{ id: 1, outcome: 'Correct displayed contact value',
          surface: ['legacy/db/contact.ts', 'legacy/services/contact.ts', 'components/contact.tsx'],
...
function splitFixture() {
  ...
  f.p.assignments[0].plan.chunks[0].surface = ['legacy/db/contact.ts']
...
test('same-file different-symbol writers conflict', () => {
  const f = splitFixture(); f.context.findings[1].seams = ['legacy/db/contact.ts']
  f.p.assignments[0].plan.chunks[0].surface = ['legacy/db/contact.ts#first']
...
test('all required findings must be assigned', () => {
  const f = fixture(); f.context.findings.push({id:'missing', required:true, seams:['legacy/db/missing.ts']}); rejects(f, /Unassigned required/)
})
test('adjacent findings cannot expand the accepted story', () => {
  const f = fixture(); f.context.findings.push({id:'later', required:false, seams:['legacy/db/later.ts']})
...
test('difficulty scorer stays advisory', () => {
  const {p, context} = fixture(); p.decision = 'SMITH'; p.size = 'MEDIUM'
  const files = Array.from({length:20}, (_,i) => 'legacy/db/f' + i + '.ts')
