...
import { reduceSplit, splitJoinHoldReasons, type SplitOutcome } from '@/legacy/workflow_app/forge/split-join'
...
test('siblings claiming the same output key block the join', () => {
  const reasons = splitJoinHoldReasons({
    expectedIds: ['a', 'b'],
    outcomes: [
      { ...ok('a'), outputKeys: ['legacy/workflow_app/forge/x.ts'] },
      { ...ok('b'), outputKeys: ['legacy/workflow_app/forge/x.ts'] },
    ],
  })
