...
} from '@/legacy/workflow_app/forge/forge-split-handoff'
import { isChangeAllowed } from '@/legacy/workflow_app/forge/smith-contract'
...
function context(splitEnabled: boolean, maxSmiths: number): RoutingContext {
  return {
    findings: [
      { id: 'f1', required: true, seams: ['legacy/workflow_app/forge/a.ts'] },
      { id: 'f2', required: true, seams: ['legacy/workflow_app/forge/b.ts'] },
    ],
    evidenceRefs: ['architect_brief'],
    splitEnabled,
    maxSmiths,
...
function smithMedium(): LeadProposal {
  return {
    ...
    reason: 'one smith',
    assignments: [assignment('a', 'f1', ['legacy/workflow_app/forge/a.ts'])],
    mergeChecks: [proof],
  }
}
...
test('SPLIT while disabled is rejected with an explicit recut reason', () => {
  const proposal: LeadProposal = {
    ...
    reason: 'split',
    assignments: [
      assignment('a', 'f1', ['legacy/workflow_app/forge/a.ts']),
      assignment('b', 'f2', ['legacy/workflow_app/forge/b.ts']),
    ],
    mergeChecks: [proof],
  }
...
test('MEDIUM + one SMITH while split is off is still legal when findings fit', () => {
  const ctx: RoutingContext = {
    findings: [{ id: 'f1', required: true, seams: ['legacy/workflow_app/forge/a.ts'] }],
    evidenceRefs: ['architect_brief'],
    splitEnabled: false,
    maxSmiths: 1,
    allowedProofs: [proof],
...
test('smithContractFromAssignment carries sibling surfaces into prohibitedScope (machine-enforced)', () => {
  const own = assignment('a', 'f1', ['legacy/workflow_app/forge/a.ts'])
  const siblingB = assignment('b', 'f2', ['legacy/workflow_app/forge/b.ts'])
  // A sibling that ALSO declares this child's own surface must not self-forbid.
  const siblingC = assignment('c', 'f3', [
    'legacy/workflow_app/forge/c.ts',
    'legacy/workflow_app/forge/a.ts',
  ])
  ...
  assert.deepEqual(withSiblings.contract.prohibitedScope, [
    'legacy/workflow_app/forge/b.ts',
    'legacy/workflow_app/forge/c.ts',
  ])
  assert.ok(!withSiblings.contract.prohibitedScope.includes('legacy/workflow_app/forge/a.ts'))
  assert.deepEqual(withSiblings.errors, [])

  // Machine enforcement: a sibling surface is denied, the own surface is allowed.
  assert.equal(isChangeAllowed(withSiblings.contract, 'legacy/workflow_app/forge/b.ts'), false)
  assert.equal(isChangeAllowed(withSiblings.contract, 'legacy/workflow_app/forge/c.ts'), false)
  assert.equal(isChangeAllowed(withSiblings.contract, 'legacy/workflow_app/forge/a.ts'), true)

  // Existing derivations are unchanged by adding siblings.
  assert.deepEqual(withSiblings.contract.allowedScope, ['legacy/workflow_app/forge/a.ts'])
...
test('a sibling surface that is an ANCESTOR of an own surface does not self-forbid the child', () => {
  const own = assignment('a', 'f1', ['legacy/workflow_app/forge/sub/a.ts'])
  ...
  const siblingDir = assignment('b', 'f2', ['legacy/workflow_app/forge/sub'])
  const siblingDirSlash = assignment('c', 'f3', ['legacy/workflow_app/forge/sub/'])
  const siblingOther = assignment('d', 'f4', ['legacy/workflow_app/forge/other.ts'])
  ...
  assert.deepEqual(contract.prohibitedScope, ['legacy/workflow_app/forge/other.ts'])
  // The child can still edit its own file even though a sibling declared the parent dir.
  assert.equal(isChangeAllowed(contract, 'legacy/workflow_app/forge/sub/a.ts'), true)
  assert.equal(isChangeAllowed(contract, 'legacy/workflow_app/forge/other.ts'), false)
})
