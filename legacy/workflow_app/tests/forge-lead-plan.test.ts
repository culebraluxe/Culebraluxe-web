...
} from '@/legacy/workflow_app/forge/forge-lead-plan'
...
test('lead plan: parses the work-order vocabulary (scope/acceptance/postconditions)', () => {
  const wo =
    'LEAD_PLAN: {"size":"SMALL","chunks":[{"id":1,"scope":["legacy/workflow_app/forge/forge-ready-gate.ts#storyReadyToRunReasons"],"acceptance":"pnpm exec tsx --test workflow_app/tests/forge-ready-gate.test.ts","preconditions":[],"postconditions":"zero-command recipe -> missing-assay-plan"}]}'
  ...
  assert.deepEqual(p!.chunks[0].surface, [
    'legacy/workflow_app/forge/forge-ready-gate.ts#storyReadyToRunReasons',
  ])
