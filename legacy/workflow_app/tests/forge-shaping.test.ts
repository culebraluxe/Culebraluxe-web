...
test('SHAPE Case A: several observations, ONE seam -> SINGLE bounded Smith unit', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('f1', ['legacy/workflow_app/forge/'], 'false-success handling'),
      req('f2', ['legacy/workflow_app/forge/forge-facts.ts'], 'typed gate facts'),
      req('f3', ['legacy/workflow_app/forge/forge-role-mapping.ts'], 'directory scope normalize'),
    ],
  })
...
test('SHAPE Case B: two independent required seams -> SPLIT(2) bounded children', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('forge-layer', ['legacy/workflow_app/forge/forge-facts.ts'], 'forge seam fix'),
...
test('SHAPE Case C: adjacent discovery never enlarges the active Smith unit', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('required-fix', ['legacy/workflow_app/forge/forge-facts.ts'], 'the required fix'),
...
test('SHAPE Case D: a chosen unit owns only its own seams (repair stays narrow)', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('forge', ['legacy/workflow_app/forge/forge-facts.ts'], 'forge seam'),
  ...
  assert.ok(forgeScope.includes('legacy/workflow_app/forge/forge-facts.ts'))
...
test('SHAPE: explicit SPLIT_CHILD is honored even when seams overlap (topology, not packet size)', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('a', ['legacy/workflow_app/forge/'], 'core change', 'SAME_UNIT'),
      req('b', ['legacy/workflow_app/forge/'], 'independent child', 'SPLIT_CHILD'),
    ],
  })
  assert.equal(decision.mode, 'SPLIT')
  assert.equal(decision.units.length, 2)
...
test('SHAPE: informational NOTE findings are notes, not follow-ups', () => {
  const decision = shapeArchitectFindings({
    findings: [
      req('fix', ['legacy/workflow_app/forge/forge-facts.ts'], 'required fix'),
...
test('SHAPE: required HOLD stops execution; lead maps it to HOLD', () => {
  const decision = shapeArchitectFindings({
    findings: [req('ambiguous', ['legacy/workflow_app/forge/'], 'too ambiguous to execute', 'HOLD')],
  })
...
test('SHAPE integration: multi-seam Architect evidence (marker) is shaped before Smith', () => {
  ...
  const evidence = findingsMarker([
    req('forge-seam', ['legacy/workflow_app/forge/'], 'forge findings'),
...
test('SHAPE gate: correct SPLIT:2 is accepted; a wrong split count is refused', () => {
  const findings = [
    req('a', ['legacy/workflow_app/forge/'], 'forge seam'),
...
test('SHAPE gate: a plain smith node cannot launch under a SPLIT/HOLD shape', () => {
  const multi = shapeArchitectFindings({
    findings: [req('a', ['legacy/workflow_app/forge/'], 'a'), req('b', ['agent-runtime/'], 'b')],
  })
...
test('SHAPE gate: single cohesive unit launches one bounded smith/repair smith', () => {
  const single = shapeArchitectFindings({ findings: [req('core', ['legacy/workflow_app/forge/'], 'core')] })
