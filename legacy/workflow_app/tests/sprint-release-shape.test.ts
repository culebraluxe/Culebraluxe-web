...
import { recordForgeBatchReleaseReceipt } from '@/legacy/db/forge-workflow-evidence'
...
test('sprint shape: a batch story records the deferral, so no per-story deploy is ever demanded', () => {
  ...
  const runner = readFileSync(new URL('legacy/workflow_app/forge/agent-runtime-role-runner.ts', REPO_ROOT), 'utf8')
...
test('sprint shape: the release is ONE receipt per batch, stamped across every story, and refused without it', async () => {
  ...
  const source = readFileSync(new URL('legacy/db/forge-workflow-evidence.ts', REPO_ROOT), 'utf8')
