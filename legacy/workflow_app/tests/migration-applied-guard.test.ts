...
const MIGRATION = 'legacy/db/migrations/179_forge_kind_policy.sql'
...
describe('migration-applied guard', () => {
  ...
  it('names EVERY unapplied file, not just the first', () => {
    const assessment = assessMigrationApplied({
      changedPaths: ['legacy/db/migrations/179_a.sql', 'legacy/db/migrations/180_b.sql'],
      ledgerFilenames: ['legacy/db/migrations/179_a.sql'],
    })
    assert.equal(assessment.ok, false)
    assert.deepEqual(assessment.unapplied, ['legacy/db/migrations/180_b.sql'])
  ...
  it('only db/migrations/*.sql paths count as migrations', () => {
    assert.deepEqual(
      migrationFilesInChangeSet([
        'legacy/db/migrations/179_a.sql',
        'legacy/db/migrations/179_a.sql',
        'legacy/db/seeds/001_seed.sql',
        'legacy/db/migrations/README.md',
        'app/page.tsx',
      ]),
      ['legacy/db/migrations/179_a.sql'],
    )
