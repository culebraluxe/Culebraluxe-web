...
} from '@/legacy/workflow_app/forge/forge-shaping'
import {
  handoffToFindings,
  parseArchitectHandoff,
  renderArchitectHandoff,
  type ArchitectHandoff,
} from '@/legacy/workflow_app/forge/agents/architect-handoff'
...
test('two disjoint new proofs under one tests directory split and each unit carries its own proof', () => {
  const findings = [
    req('unit-a', ['legacy/workflow_app/tests'], ['legacy/workflow_app/tests/proof-a.test.ts']),
    req('unit-b', ['legacy/workflow_app/tests'], ['legacy/workflow_app/tests/proof-b.test.ts']),
  ]
  ...
  assert.ok(unitA.seams.includes('legacy/workflow_app/tests/proof-a.test.ts'), 'unit A is handed its own proof')
  assert.ok(!unitA.seams.includes('legacy/workflow_app/tests/proof-b.test.ts'), 'unit A does not own the sibling proof')
  assert.ok(unitB.seams.includes('legacy/workflow_app/tests/proof-b.test.ts'), 'unit B is handed its own proof')

  const scopeA = smithScopeForUnit(decision, unitA.id)
  assert.ok(scopeA.includes('legacy/workflow_app/tests/proof-a.test.ts'))
})

test('a required proof outside its finding seams is refused at shaping by finding id', () => {
  const decision = shapeArchitectFindings({
    findings: [req('bad-proof', ['legacy/workflow_app/forge/forge-shaping.ts'], ['legacy/workflow_app/tests/orphan.test.ts'])],
  })
  assert.equal(decision.mode, 'HOLD')
  assert.ok(decision.reason.includes('bad-proof'), 'the refusal names the finding')
  assert.ok(decision.reason.includes('legacy/workflow_app/tests/orphan.test.ts'), 'the refusal names the proof')
})

test('two findings editing the same file still overlap and are refused', () => {
  const findings = [
    req('same-a', ['legacy/workflow_app/tests'], ['legacy/workflow_app/tests/same.test.ts']),
    req('same-b', ['legacy/workflow_app/tests'], ['legacy/workflow_app/tests/same.test.ts']),
  ]
  const decision = shapeArchitectFindings({ findings })
...
test('the architect handoff carries a finding proof into the work order', () => {
    ...
    findings: [
      {
        ...
        preconditions: [],
        scope: ['legacy/workflow_app/tests'],
        proofs: ['legacy/workflow_app/tests/proof-seam.test.ts'],
        postconditions: [],
        classes: [],
        risks: [],
        hint: 'SAME_UNIT',
  ...
  assert.deepEqual(parsed.findings[0].proofs, ['legacy/workflow_app/tests/proof-seam.test.ts'])
  const findings = handoffToFindings(parsed)
  assert.deepEqual(findings[0].proofs, ['legacy/workflow_app/tests/proof-seam.test.ts'])
  const order = smithWorkOrdersFromFindings(['proof-owner'], parsed)
  assert.ok(order.includes('legacy/workflow_app/tests/proof-seam.test.ts'), 'the work order names the proof')
})
