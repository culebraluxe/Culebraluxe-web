...
import { mapForgeWorkflowEvidence } from '@/legacy/db/forge-workflow-evidence'
import { projectForgeGateFacts } from '@/legacy/workflow_app/forge/forge-facts'
...
test('ENG-FORGE-SHAPE-01: findings JSONB normalizes into the durable Architect snapshot', () => {
  const raw = [
    { id: 'forge', summary: 'forge seam', required: true, seams: ['legacy/workflow_app/forge/'], hint: 'SAME_UNIT' },
  ...
  assert.deepEqual(fromArray.findings?.[0].seams, ['legacy/workflow_app/forge/'])
