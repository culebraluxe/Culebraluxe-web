...
import type { SmeParticipantCandidate } from '@/legacy/workflow_app/sme-orchestration'
...
export async function reconcileTaskMaterialization(): Promise<{
  ...
  const { materializeEngineTask } = await import('@/legacy/workflow_app/task-materialization')
  const { orchestrateSmeTaskCore } = await import('@/legacy/workflow_app/sme-orchestration')
