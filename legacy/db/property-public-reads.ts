...
import type { DatabaseGateway } from "@/legacy/db/database-gateway"
import type {
  PropertyFilterInput,
  PropertyIntro,
  PropertySummary,
...
/**
 * The DB seam for public inventory reads: the Result-returning gateway, so a read
 * degrades to a failed Result instead of rejecting the page (DB-HARDEN-01C). Never
 * a raw driver throw. NOTE: this sits behind SqlPropertyRepository — the public
 * surfaces reach it through the Property service, not directly.
 */
export type PropertyReadExecutor = Pick<DatabaseGateway, "query" | "queryOne">
...
export async function getPropertyIntroById(
  propertyId: string,
  executor: PropertyReadExecutor = db,
): Promise<Result<PropertyIntro | null>> {
  // DB-HARDEN-01C — public read: never throw a DB failure; return a Result.
  const r = await executor.queryOne<{ id: string; name: string; location: string | null }>`
...
export async function getProperties(opts: {
  /**
   * HARDEN-05 — publicOnly: true returns ACTIVE LISTING inventory only: an active
   * listing (is_active_listing) that is published (is_published) and not archived.
   * A property that is only a known place (an Apple Contacts address, or a listing
   * we stopped marketing) is not inventory and never appears on a public surface.
   * false (default) returns the working lifecycle set (active/coming_soon/
   * under_contract, non-archived) used by internal portal pickers that may also
   * work with staged rows. The PUBLIC surfaces MUST pass publicOnly: true.
   */
  publicOnly?: boolean
} = {},
  executor: PropertyReadExecutor = db,
  ...
  const r = await executor.query<PropertySummaryRow>`
    select
      p.id,
      p.name,
      p.slug,
...
export async function getFilteredProperties(
  ...
  const propertyRowsR = await executor.query<PropertySummaryRow>`
    select
      p.id,
      p.name,
      p.slug,
  ...
  const viewRowsR = await executor.query`
    select
      has_ocean_view,
      has_bay_view,
      has_beach_view,
...
export async function getPublicPropertySlugs(
  ...
  const r = await executor.query<{ slug: string }>`
    select slug
    from property
    where archived_at is null
      and slug is not null
...
export async function getPropertyBySlug(
  ...
  const r = await executor.query<PropertyRow>`
    select
      p.id,
      p.name,
      p.slug,
