...
import { createTransactionDocument } from '@/legacy/db/transaction-document'
import {
  createContractIssuedEvidence,
  getFormContractId,
  getPriorContractIssuedDocument,
} from '@/legacy/db/contract-issued-document'
import { getFormInstance, updateFormInstance } from '@/legacy/db/form-service-repository'
import { listFormSignerPeople } from '@/legacy/db/form-signer'
...
import { resolveBrokerSignatureForIssuance } from '@/legacy/db/broker-signature'
...
export async function getMediaBytes(
  ...
  const q = execute ?? (await import('@/legacy/db/client')).sql
  const rows = await q`
