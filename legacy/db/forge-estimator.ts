import {
  ...
  type WorkRisk,
  type WorkToolkit,
} from '@/legacy/workflow_app/forge/forge-estimator'
import {
  applySurfaceMultiplier,
  ripwireSurfaceFromPack,
  surfaceMultiplier,
} from '@/legacy/workflow_app/forge/forge-ripwire-surface'
...
export async function createWorkEstimate(
  ...
  const rows = await q`
    insert into work_estimate (
      ...
      seams_count, acceptance_count, points, estimated_tokens, estimated_widgets,
      ...
      ${forecast.points}, ${forecast.estimatedTokens}, ${forecast.estimatedWidgets},
      ${forecast.estimatedMinutes}, ${forecast.estimatedSloc}, ${forecast.risk}, 'forecast',
      ${score}, ${surface?.files ?? null}, ${surface?.ccxTotal ?? null}
    )
    returning id, points, estimated_tokens, estimated_widgets, estimated_minutes,
...
export async function listWorkEstimates(
  ...
  const q = execute ?? (await executor())
  const rows = await q`
    select id, points, estimated_tokens, estimated_widgets, estimated_minutes,
...
/** Real actuals already recorded on this story's runs (tokens / widgets / minutes),
 * the calibration half of the estimator. Half the data is better than none. */
export async function runActualTotalsForStory(
  storyId: string,
  execute?: QueryExecutor,
): Promise<{ tokens: number; costWidgets: number; minutes: number } | null> {
  const q = execute ?? (await executor())
  const rows = await q`
    select coalesce(sum(coalesce(tokens_input,0) + coalesce(tokens_output,0)), 0)::int as tokens,
           coalesce(sum(cost_widgets), 0) as cost_widgets,
           coalesce(sum(extract(epoch from (coalesce(ended_at, started_at) - started_at)) / 60), 0) as minutes
    from storyboard_story_run
    where story_id = ${storyId}
  `
  const row = rows[0] as { tokens: number; cost_widgets: number; minutes: number } | undefined
  if (!row) return null
  return { tokens: Number(row.tokens), costWidgets: Number(row.cost_widgets), minutes: Number(row.minutes) }
}
