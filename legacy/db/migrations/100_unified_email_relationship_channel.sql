-- 100_unified_email_relationship_channel.sql
-- Extend the locked Cloze-style Client relationship-memory contract:
-- Gmail and Apple-hosted iCloud Mail normalize into one Person x Email row, with
-- the newest bounded subject and that context event's own timestamp/direction.
...
with comm as (
    select
        ...
        case
            when ev.source in ('gmail_contacts', 'gmail', 'icloud_mail') then 'email'
            else ev.source
        end as source,
...
context_candidates as (
    select
        i.person_id,
        case
            when i.channel = 'email' then 'email'
            else coalesce(i.source_system, '')
        end as source,
        i.direction,
        i.occurred_at,
...
select
    ...
    case
        ...
        when a.source = 'email' then 'email'
        else a.source
    end as channel,
    a.first_observed_at,
    a.last_contact_at,
...
-- Recreate the dependent directory with the current 095 search/name behavior.
...
with base as (
    select
        ...
        trim(coalesce(concat_ws(' ', p.display_name, p.location, identity.search_text), '')) as search_text
    from person p
    ...
    left join lateral (
        select coalesce(
            (select max(rc.last_contact_at) from mv_client_relationship_channels rc where rc.person_id = p.id),
            (select max(i.occurred_at) from interaction i where i.person_id = p.id)
        ) as occurred_at
    ) latest on true
    left join lateral (
...
create unique index mv_client_directory_pk on mv_client_directory (person_id);
create index mv_client_directory_sort on mv_client_directory (name_sort_priority desc, display_name asc, person_id asc);
...
create index mv_client_directory_search on mv_client_directory using gist (search_text gist_trgm_ops);
